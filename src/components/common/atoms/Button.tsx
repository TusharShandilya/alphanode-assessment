...
const Button = ({
  ...
  const css = clsx(
    "ext-white hover:bg-blue-800 focus:ring-4  font-medium rounded-lg text-sm px-5 py-2.5 m-2  focus:outline-none dark:focus:ring-blue-800 cursor-pointer",
    {
      ...
      "focus:ring-green-300  dark:bg-green-600 dark:hover:bg-green-700 text-white  bg-green-700":
        variant === "success",
      "focus:ring-red-300  dark:bg-red-600 dark:hover:bg-red-700 text-white  bg-red-700":
        variant === "danger",
  ...
  return (
    <button className={css} aria-label={children} {...rest}>
      {leadIcon && <span aria-hidden={true}>{leadIcon}</span>}
      {iconOnly ? "" : children}
    </button>
  );
};
